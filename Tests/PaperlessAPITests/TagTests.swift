...
@Test func decodeTagWithPermissions() async throws {
...
""".data(using: .utf8)!
    ...
    #expect(tag.textColor == Color.black)
    #expect(tag.match == "")
    #expect(tag.matchingAlgorithm == .matchAny)
    #expect(tag.isInsensitive)
    #expect(!(tag.isInboxTag))
    #expect(tag.documentCount == 8)
    #expect(tag.owner == nil)
    #expect(tag.permissions?.view.users == [])
    #expect(tag.permissions?.view.groups == [])
    #expect(tag.permissions?.change.users == [])
    #expect(tag.permissions?.change.groups == [])
}
